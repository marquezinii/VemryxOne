...
public sealed partial class LocalizedInterfaceContractTests
{
    ...
    public void LocalizedXamlBindings_ResolveInEnglishAndPortuguese()
    {
        ...
        var sources = new[]
        {
            Path.Combine(root, "src", "Vemryx.One.App", "MainWindow.xaml"),
            Path.Combine(root, "src", "Vemryx.One.App", "Views", "BugReportWindow.xaml"),
            Path.Combine(root, "src", "Vemryx.One.App", "Views", "PrivacyConsentWindow.xaml"),
            Path.Combine(root, "src", "Vemryx.One.App", "Views", "ReleaseNotesWindow.xaml"),
    ...
    {
        var root = TestHelpers.FindRepositoryRoot();
        var pageDirectory = Path.Combine(root, "src", "Vemryx.One.App", "Views", "Pages");
    ...
    public void Dashboard_FocusesOnStatusInsteadOfDuplicatingOptimizerControls()
    {
        ...
        var viewModel = File.ReadAllText(Path.Combine(root, "src", "Vemryx.One.App", "ViewModels", "MainViewModel.Diagnostics.cs"));
    ...
    public void BugReportAndCopyright_AreInSettingsInsteadOfAGlobalFooter()
    {
        ...
        var root = TestHelpers.FindRepositoryRoot();
        var mainWindowPath = Path.Combine(root, "src", "Vemryx.One.App", "MainWindow.xaml");
    ...
    public void ReleaseNotesLinkButton_UsesLinkButtonStyleInsteadOfTheDefaultButtonChrome()
    {
        ...
        var root = TestHelpers.FindRepositoryRoot();
        var document = XDocument.Load(
            Path.Combine(root, "src", "Vemryx.One.App", "Views", "Pages", "OverviewPage.xaml"));
        XNamespace presentation = "http://schemas.microsoft.com/winfx/2006/xaml/presentation";
