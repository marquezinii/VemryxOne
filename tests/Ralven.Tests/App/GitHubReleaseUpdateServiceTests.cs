...
public sealed class GitHubReleaseUpdateServiceTests
{
    ...
    public async Task CheckForUpdate_UsesOfficialEndpointHeadersAndReturnsNewerRelease()
    {
        ...
        {
            Assert.Equal(
                "https://api.github.com/repos/marquezinii/Ralven/releases/latest",
                request.RequestUri?.AbsoluteUri);
            Assert.Contains("Ralven-Updater/1.0", request.Headers.UserAgent.ToString());
            Assert.Contains(
        ...
        Assert.Equal("Ralven-Setup-1.2.3-win-x64.exe", update.AssetName);
    ...
    [InlineData("https://github.com/marquezinii/Ralven/releases/tag/v2.0.1")]
    [InlineData("http://github.com/marquezinii/Ralven/releases/tag/v2.0.0")]
    ...
    [Theory]
    [InlineData("Ralven-Setup.exe")]
    [InlineData("Ralven-Setup-1.9.9-win-x64.exe")]
    [InlineData("Ralven-Setup-2.0.0-preview-win-x64.exe")]
            ...
            assets = new[]
            {
                new
                {
                    ...
                    browser_download_url =
                        $"https://github.com/marquezinii/Ralven/releases/download/v2.0.0/{assetName}",
                },
            },
        });
        using var service = CreateService(ManifestOnlyHandler(json), scope.Path);
        ...
        string digest)
    {
        ...
        var json = JsonSerializer.Serialize(new
        {
            ...
            assets = new[]
            {
                new
                {
                    name = "Ralven-Setup-2.0.0-win-x64.exe",
                    state,
                    size,
                    digest,
    ...
    [Theory]
    [InlineData("http://github.com/marquezinii/Ralven/releases/download/v2.0.0/Ralven-Setup-2.0.0-win-x64.exe")]
    [InlineData("https://evil.example/marquezinii/Ralven/releases/download/v2.0.0/Ralven-Setup-2.0.0-win-x64.exe")]
    [InlineData("https://github.com.evil.example/marquezinii/Ralven/releases/download/v2.0.0/Ralven-Setup-2.0.0-win-x64.exe")]
    [InlineData("https://github.com/marquezinii/Other/releases/download/v2.0.0/Ralven-Setup-2.0.0-win-x64.exe")]
    [InlineData("https://github.com/marquezinii/Ralven/releases/download/v9.0.0/Ralven-Setup-2.0.0-win-x64.exe")]
    [InlineData("https://github.com/marquezinii/Ralven/releases/download/v2.0.0/Other.exe")]
    [InlineData("https://github.com/marquezinii/Ralven/releases/download/v2.0.0/%2e%2e/Ralven-Setup-2.0.0-win-x64.exe")]
    [InlineData("https://github.com/marquezinii/Ralven/releases/download/v2.0.0/Ralven-Setup-2.0.0-win-x64.exe?download=1")]
    ...
    public async Task CheckForUpdate_RejectsManifestRedirectEvenWhenDestinationUsesGithub()
    {
        using var scope = new TemporaryDirectory();
        var handler = new StubHttpMessageHandler((request, _) => Task.FromResult(
            RedirectResponse(request, "https://github.com/marquezinii/Ralven/releases/latest")));
        using var service = CreateService(handler, scope.Path);
    ...
    public async Task DownloadUpdate_FollowsOnlyAllowlistedRedirectAndVerifiesBeforeAtomicMove()
    {
        ...
        Assert.Equal(
            Path.Combine(scope.Path, "2.4.0", "Ralven-Setup-2.4.0-win-x64.exe"),
    ...
    public async Task DownloadUpdate_RejectsHashMismatchAndRemovesPartialFile()
    {
        ...
        Assert.False(File.Exists(Path.Combine(scope.Path, "2.0.0", "Ralven-Setup-2.0.0-win-x64.exe")));
        Assert.Empty(Directory.GetFiles(scope.Path, "*.part", SearchOption.AllDirectories));
    }
    ...
    [Theory]
    [InlineData("../Ralven-{version}.exe")]
    [InlineData("sub/Ralven-{version}.exe")]
    [InlineData("Ralven-{version}.exe:evil")]
    [InlineData("not-an-installer-{version}.zip")]
    [InlineData("Ralven.exe")]
    [InlineData("Ralven-{version}-{version}.exe")]
    ...
    private static string OfficialDownloadUrl(string tag) =>
        $"https://github.com/marquezinii/Ralven/releases/download/{tag}/{AssetName(tag)}";

    private static string OfficialReleaseNotesUrl(string tag) =>
        $"https://github.com/marquezinii/Ralven/releases/tag/{tag}";
