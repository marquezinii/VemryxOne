...
/// <c>%LOCALAPPDATA%\Ralven\Transactions\{id}.json</c>. Journals survive
...
public sealed class PersistedEnumContractTests
{
    ...
    private static string CreateTemporaryDirectory()
    {
        var directory = Path.Combine(
            Path.GetTempPath(),
            "RalvenJournalContract_" + Guid.NewGuid().ToString("N"));
        ...
        where TEnum : struct, Enum
    {
        Assert.Equal(
            $"\"{expected}\"",
            JsonSerializer.Serialize(value, RalvenJson.Options));
    }
}
