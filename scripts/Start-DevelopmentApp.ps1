...
$solutionPath = Join-Path $workspace 'Ralven.slnx'
...
if ($null -eq $dotnet) {
    throw 'The .NET SDK is required to run the Ralven development launcher.'
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    [void][System.Windows.MessageBox]::Show(
        "O Ralven não pôde ser preparado para execução. O registro do build está em:`n$buildLogPath",
        'Ralven - Desenvolvimento',
...
# See Ralven.App/Services/AppEnvironment.cs.
$env:RALVEN_ENVIRONMENT = 'Development'
